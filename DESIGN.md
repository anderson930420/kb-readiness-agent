...
| Answer behavior | 小型 deterministic router 先處理 standalone greeting、thanks、empty input、app intro 與 clearly unrelated general queries；company/support/policy/pricing/service 與 ambiguous queries 維持預設 extractive：回傳最高排名 chunk 原文，或依檢索門檻與已知 unsupported policy evidence 拒答；optional generative mode 不改變預設路徑 |
...
### 4.0 Positive KB admission

`answer_question` 在 retrieval 前以 normalized exact allowlist 辨識 standalone greeting、
thanks 與 app-introduction questions，並以保守的 deterministic patterns 辨識
weather、trivia、recipes、sports、creative requests 等 clearly unrelated general queries。
前者回傳 `response_type=non_kb_chitchat` capability message；後者回傳
`response_type=out_of_scope_general` scope refusal。兩者皆設為
`groundedness.status=not_applicable`，不讀 index、不呼叫 provider，且 citations /
retrieved chunks 為空。

空白、標點、代名詞、function words 為主或不完整的 query 會在 retrieval 前回到 task
boundary。Company、support、policy、pricing、service 等明確 KB scope signals 固定進入原本
RAG path；其他具有足夠語意內容的 query 先用 retrieval top score 做 conservative relevance
probe（lexical 3.0、dense 0.45、hybrid 0.70），未達門檻不會進入 answer generation 或
validator。citation exemption 只適用於兩種固定 canned response。

Ask Mode uses positive KB admission: queries enter RAG only when they contain explicit
support-domain signals or pass a retrieval relevance probe; otherwise they receive a
task-boundary response before retrieval/generation.
