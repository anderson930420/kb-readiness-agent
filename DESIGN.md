...
Day 3 hardens the retrieval and refusal foundation for Ask Mode. It is not yet
...
## 2. Current implementation status（Day 3）
...
| Tests | Day 1/2 regression 與 Day 3 answer reliability tests |
...
### Ask Mode — Day 3 active

已提供 CLI retrieval、結構化抽取式 answer、拒答、chunk citation、deterministic
groundedness 與正式 eval gate。這是目前唯一可執行的 product mode。
...
## 6. Evaluation（Day 3 actual results）
...
### Day 3 hybrid gate results (`k=3`, 20 active cases)
...
因此目前的 Day 3 retrieval gate 結論是：hybrid 在中英文 20 個 active cases 上都達到
`source_hit@3 = 20/20` 與 `section_hit@3 = 20/20`。這個結果只適用於目前的小型、人工整理
eval set，不代表 production accuracy。

Day 3 answer-level gate 額外得到：
...
目前 18 tests 通過，涵蓋 34-chunk corpus-only ingestion、Day 1 lexical smoke cases、
