...
Day 2 delivers the retrieval and refusal foundation for Ask Mode. It is not yet
the full audit product described by the longer-term product framing below.
...
目前範圍刻意收斂在 Ask Mode：先確認雙語檢索、證據選擇、抽取式回答與拒答行為
可靠，再往 Audit Mode 與 Change Impact Mode 擴充。

---

## 2. Current implementation status（Day 2）
...
| Answer behavior | 回傳最高排名 chunk 的原文，或依檢索門檻與已知 unsupported policy evidence 拒答 |
| Citations | chunk-level citation：文件、section、section slug、chunk ID |
| Diagnostics | `eval/run_eval.py` 比較 lexical / dense / hybrid 的中英文 retrieval 結果 |
| Tests | Day 1 regression 與 Day 2 retrieval/cache/refusal tests 均通過 |

### 尚未實作

以下能力不屬於目前 Day 2 完成範圍：
...
- deterministic groundedness validator 或正式 `AnswerResult` schema
- Streamlit UI
- Audit Mode readiness report
- Change Impact Mode 與 conflict detection
...
flowchart LR
    ...
    X --> Z[Answer + chunk citation]
    F --> Z
```

Ask Mode 支援三個可選 backend：`lexical`、`dense`、`hybrid`。為保留 Day 1
相容性，CLI 的程式預設仍是 `lexical`；目前建議的 Day 2 使用方式是明確指定
`--retriever hybrid`。
...
這是 retrieval contract，不應解讀為尚未實作的完整 `AnswerResult` contract。
...
### Ask Mode — Day 2 active

已提供 CLI retrieval、抽取式 answer、拒答與 chunk citation。這是目前唯一可執行的
product mode。

### Audit Mode — planned Day 4

目標是把 evaluation output 產品化成 Knowledge Base Readiness Report，例如 retrieval
coverage、refusal review、knowledge gaps 與 launch recommendation。目前只有 bilingual
retrieval diagnostics，尚未產生 readiness report，也沒有 UI。
...
## 6. Evaluation（Day 2 actual results）
...
每個 active case 會分別執行中文 `question` 與英文 `question_en`。Day 2 diagnostics 評估的是
retrieval，不是 LLM answer quality。
...
### Day 2 results (`k=3`, 20 active cases)
...
因此目前的 Day 2 retrieval gate 結論是：hybrid 在中英文 20 個 active cases 上都達到
`source_hit@3 = 20/20` 與 `section_hit@3 = 20/20`。這個結果只適用於目前的小型、人工整理
eval set，不代表 production accuracy。

目前沒有宣稱或量測 aggregate answer accuracy、faithfulness、groundedness、citation
correctness、latency、token usage 或 per-query cost。
...
目前 12 tests 通過，涵蓋 34-chunk corpus-only ingestion、Day 1 lexical smoke cases、
multilingual dense result schema、embedding cache reuse、hybrid determinism，以及 medical
refund refusal evidence selection。

---

## 7. Roadmap（not implemented）

### Planned Day 3 — Ask Mode evidence quality

- 評估並加入 reranker；只有實測改善後才保留。
- 設計正式 `AnswerResult` schema。
- 加入 deterministic groundedness checks。
- 將 chunk-level citation 細化為 sentence / claim-level citation。
- 評估 PyMuPDF / pdfplumber PDF ingestion；保留 Markdown path 作為 baseline。

### Planned Day 4 — Audit productization

- 將 diagnostics 轉成結構化、可保存的 readiness report。
- 建立 Audit Mode 指標與 failure-case drill-down。
- 建立 Streamlit UI，前提是 CLI 與 report schema 已穩定。
- 若引入 LLM，再加入 token、latency 與 cost observability。
