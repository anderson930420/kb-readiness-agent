...
thanks、empty input 與 app-introduction questions，並以保守的 deterministic patterns 辨識
...
Company、support、policy、pricing、service 等 KB scope signals 的優先級較高，固定進入原本
retrieval、groundedness 與 optional generation validator；無法明確分類的 query 也預設走
KB path。citation exemption 只適用於兩種固定 canned response。Readiness runner 在 retrieval
前套用相同分類，排除 non-KB rows，避免一般問題進入 KB readiness metrics。
